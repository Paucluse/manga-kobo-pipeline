...
- **卷号**：合集目录下一层子项名如果是明确卷号（如 `1/`、`02/`、`Vol_03.zip`、`第4卷.cbz`），该卷号优先级最高。外部平台返回的卷号不会覆盖这个结构化卷号，避免整套书刮到同一个平台条目时全部变成第 1 卷。
...
- **快捷纠错与入库**：支持页面内直接修改字段并重新推进入库，或对仍在 `inbox` 中的源文件重置管线，让自动流程从文件名解析开始完整重跑。重置会把旧归档/KEPUB 产物先移到 `processing/reset-backups`。
- **已入库记录处理**：如果源文件已被成功导入后从 `inbox` 删除，不能再从头重置；此时应使用重新刮削或重新导入来修正 Komga 中已有书籍。
